...
class coro_alloc {
  ...
  static std::size_t __alloc_size(std::size_t size) noexcept {
    return reinterpret_cast<std::uintptr_t>(__alloc_address<Alloc>(0, size)) +
           sizeof(Alloc);
  }
  ...
  static void __deallocator(void *ptr, std::size_t size) noexcept {
    ...
    if constexpr (is_stateless<Alloc>) {
      ...
      Alloc *a_ptr =
          __alloc_address<Alloc>(reinterpret_cast<std::uintptr_t>(ptr), size);
  ...
  static void *__allocate(const Alloc &a, std::size_t size) noexcept {
    using rebind_a_t =
        typename std::allocator_traits<Alloc>::template rebind_alloc<block_t>;
    using rebind_attr_t =
        typename std::allocator_traits<Alloc>::template rebind_traits<block_t>;
...
// , __coro_alloc::coro_alloc
{
  ...
  stdexec::inplace_stop_token get_token() const noexcept { return _stop_token; }

  bool stop_requested() const noexcept { return _stop_token.stop_requested(); }
  ...
  stop_callback_t(const StopToken &parent_token,
    ...
    if constexpr (stdexec::unstoppable_token<stop_token_type>)
      return;
                                      ...
                                      stdexec::inplace_stop_token>) {
      _callback.emplace<1>(parent_token, forward_stop_request_t{&self_source});
    } else {
...
public:
  ...
  inline_task(inline_task &&other) noexcept : _h{std::exchange(other._h, {})} {}
