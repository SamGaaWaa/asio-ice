...
static const auto MDNS_ENDPOINT =
    mdns::net::ip::udp::endpoint{MDNS_ADDR, MDNS_PORT};

namespace mdns {

const char *version() noexcept { return "cppmdns-0.0.1"; }

struct query : std::enable_shared_from_this<query>,
               boost::intrusive::set_base_hook<
                   boost::intrusive::link_mode<boost::intrusive::safe_link>> {
    query(std::string name, dns::record_type type,
          std::shared_ptr<server::impl_t> server_impl, bool is_probing = false)
        : _question{std::move(name), type, dns::record_class::INTERNET},
          _server_impl{std::move(server_impl)}, _is_probing{is_probing} {
        if (!_server_impl) {
            SAMLOG_ERROR(auto sink) { sink("server_impl is null\n"); };
    ...
    make_query(std::string name, dns::record_type type,
               std::shared_ptr<server::impl_t> server_impl,
               bool is_probing = false) {
        return std::make_shared<query>(std::move(name), type,
                                       std::move(server_impl), is_probing);
    }
    ...
    auto get_future() noexcept { return _result.get_future(); }

    dns::record_type type() const noexcept { return _question.type; }

    bool is_probing() const noexcept { return _is_probing; }
  ...
  private:
    ...
    utils::shared_promise<std::expected<dns::record_t, std::error_code>>
        _result;
...
using query_set = boost::intrusive::multiset<
    query, boost::intrusive::key_of_value<query::query_comparer>>;

static query_ptr find_match_query(query_set &queries,
...
struct service_t
    ...
    void stop() noexcept { update_timeout(0); }
...
struct server::impl_t : std::enable_shared_from_this<server::impl_t> {
    impl_t(net::any_io_executor ex) noexcept
        : executor{std::move(ex)}, sock_s4{executor, net::ip::udp::v4()},
    ...
    exec::task<std::expected<std::string, std::error_code>>
    publishAddr(std::string ip, int seconds = 3600,
                std::shared_ptr<server::impl_t> self = {});

    exec::task<void> remove(std::string name,
                            std::shared_ptr<server::impl_t> self = {});
...
void query::start() {
    ...
    utils::detached_with_data(
        stdexec::starts_on(sched,
                           stdexec::when_all(this->query_loop(),
                                             _cancel.get_future() |
...
inline_task<void> query::query_loop() {
    ...
    int first_delay =
        this->_is_probing ? utils::rand(0, 250) : utils::rand(20, 120);
    ...
    while (!this->_result.empty()) {
        auto [err1, nsend1] = co_await impl.sock_s4.async_send_to(
            net::buffer(buf, *n), MDNS_ENDPOINT, asio2exec::use_sender);
        // auto [err2, nsend2] = co_await
        // impl.sock_v6->async_send(net::buffer(buf, *n),
...
service_t::service_t(dns::record_t record,
                     std::shared_ptr<server::impl_t> server_impl)
    : record{std::move(record)}, _server_impl{std::move(server_impl)},
      _timer{_server_impl->executor} {}

void service_t::start() {
    auto sched = asio2exec::scheduler{_server_impl->executor};
    utils::detached_with_data(stdexec::starts_on(sched, time_out_control()),
                              shared_from_this());
}
...
exec::task<void> service_t::time_out_control() {
    utils::scope_guard on_exit([&]() noexcept {
        if (this->is_linked())
            this->_server_impl->services.erase(
                this->_server_impl->services.iterator_to(*this));
    });

    do {
        this->_timer.expires_after(std::chrono::seconds{this->record.ttl});
        co_await (this->_timer.async_wait(asio2exec::use_sender) |
...
exec::task<std::expected<std::string, std::error_code>>
server::impl_t::queryAddr(std::string name,
                          std::shared_ptr<server::impl_t> self) {
    if (_stopped) {
    ...
    if (auto it = std::ranges::find_if(
            this->services,
            [&name](const auto &s) { return s.record.name == name; });
        ...
        if (type == dns::record_type::A) {
            co_return net::ip::address_v4{std::get<dns::rdata::a>(r.data)}
                .to_string();
        }
        co_return net::ip::address_v6{std::get<dns::rdata::aaaa>(r.data)}
            .to_string();
    }
    // Find query
    ...
    if (q) {
        ...
        // Make a query
        q = query::make_query(std::string{name}, dns::record_type::ANY, self);
        queries.insert(*q);
        q->start();
    }
    ...
    try {
        if (type == dns::record_type::A) {
            co_return net::ip::address_v4{std::get<dns::rdata::a>(addr)}
                .to_string();
        }
        co_return net::ip::address_v6{std::get<dns::rdata::aaaa>(addr)}
            .to_string();
...
server::impl_t::probe(std::string name) {
    ...
    if (!q) {
        // Make a query
        q = query::make_query(std::string{name}, dns::record_type::ANY,
...
exec::task<std::expected<std::string, std::error_code>>
server::impl_t::publishAddr(std::string ip, int seconds,
                            std::shared_ptr<server::impl_t> self) {
    if (_stopped) {
        co_return std::unexpected(
            std::make_error_code(std::errc::operation_canceled));
    ...
    dns::record_t record{.name = get_uuid() + ".local",
    ...
    try {
        ...
        if (addr.is_v4()) {
            ...
            service->record.data.emplace<dns::rdata::aaaa>(
                addr.to_v6().to_bytes());
        }
    } catch (const std::exception &e) {
        co_return std::unexpected(
    ...
    //      if (other)
    //          co_return std::error_code((int)error::service_already_exists,
    //          mdns_category());
    //  }
    service->start();
...
exec::task<void> server::impl_t::remove(std::string name,
                                        std::shared_ptr<server::impl_t> self) {
    if (_stopped) {
        co_return;
    }
...
void server::impl_t::start() {
    ...
    utils::detached_with_data(
        stdexec::starts_on(sched,
                           stdexec::when_all(server_loop(), receive_loop(),
                                             _stop.get_future() |
                                                 stdexec::continues_on(sched)) |
                               stdexec::into_variant()),
        this->shared_from_this());
...
inline_task<void> server::impl_t::server_loop() {
    char buf[4096];
    utils::scope_guard on_exit([&]() noexcept { this->stop(); });

    while (true) {
        net::ip::udp::endpoint remote;
        auto [err, n] = co_await this->sock_r4.async_receive_from(
        ...
        if (n == 0) {
            SAMLOG_WARN(auto sink) { sink("Received empty query\n"); };
            continue;
        }
        ...
        if (!msg.has_value()) {
            SAMLOG_DEBUG(auto sink) { sink("Received invalid query\n"); };
            continue;
        }
        if (msg->is_response()) {
            SAMLOG_TRACE(auto sink) {
                auto js = msg->to_json();
                sink("Received response: {}\n", js.dump(2));
            };
            ...
            if (question.is_unicast())
                for (const auto &service : this->services) {
                    if (service.record.name == question.name &&
                        (question.type == dns::record_type::ANY ||
                         service.record.type() == question.type)) {
        ...
        if (!multicast_resp.answers.empty()) {
            ...
            std::tie(err, n) = co_await this->sock_s4.async_send_to(
...
inline_task<void> server::impl_t::receive_loop() {
    ...
    while (true) {
        net::ip::udp::endpoint remote;
        auto [err, n] = co_await this->sock_s4.async_receive_from(
            net::buffer(buf, sizeof(buf)), remote, asio2exec::use_sender);
        if (err) {
            SAMLOG_WARN(auto sink) {
                char buf[256];
        ...
        auto msg = dns::message_t::parse(buf, n);
        if (!msg.has_value()) {
            SAMLOG_WARN(auto sink) { sink("Received invalid query\n"); };
            continue;
        }
        if (!msg->is_response()) {
            //
        ...
        SAMLOG_TRACE(auto sink) {
            auto js = msg->to_json();
            sink("Receive loop: Received response: {}\n", js.dump(2));
        };
        if (msg->answers.empty())
            continue;
        for (const auto &answer : msg->answers) {
...
const service_t *server::impl_t::find_service_by_ip(std::string_view ip) const {
    for (const auto &s : this->services) {
        ...
        if (type == dns::record_type::A)
            srv_ip =
                net::ip::address_v4{std::get<dns::rdata::a>(addr)}.to_string();
        else
            srv_ip = net::ip::address_v6{std::get<dns::rdata::aaaa>(r.data)}
                         .to_string();
        if (srv_ip == ip)
            return &s;
    }
...
server::query(std::string name) {
    if (!_impl) {
        SAMLOG_WARN(auto sink) { sink("Server is stopped\n"); };
...
exec::task<void> server::remove(std::string name) {
    if (!_impl) {
        SAMLOG_WARN(auto sink) { sink("Server is stopped\n"); };
        throw std::runtime_error("Server is stopped");
    }
