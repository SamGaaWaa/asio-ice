...
#include "common/numerics/divide_round.h"
#include "common/numerics/safe_conversions.h"

namespace dcsctp::utils {
...
class UnitBase {
 public:
  ...
  constexpr Unit_T RoundTo(const Unit_T& resolution) const {
    ...
    assert(std::cmp_greater(resolution.value_, 0));
 ...
 protected:
  ...
  static constexpr Unit_T FromValue(T value) {
    ...
    assert(std::cmp_greater(value, MinusInfinityVal()));
    assert(std::cmp_less(value, PlusInfinityVal()));
  ...
  static constexpr Unit_T FromFraction(int64_t denominator, T value) {
    if (Unit_T::one_sided)
      assert(std::cmp_greater_equal(value, 0));
    assert(std::cmp_greater(value, MinusInfinityVal() / denominator));
    assert(std::cmp_less(value, PlusInfinityVal() / denominator));
  ...
  constexpr typename std::enable_if<std::is_integral<T>::value, T>::type
  ToMultiple() const {
    assert(std::cmp_greater_equal(ToValue(), std::numeric_limits<T>::min() / Factor));
    assert(std::cmp_less_equal(ToValue(), std::numeric_limits<T>::max() / Factor));
