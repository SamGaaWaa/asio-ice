...
void request_test() {
	...
	std::cout << "STUN server: " << server_ep.address().to_string() << ':' << server_ep.port() << '\n';
    ...
    auto recv_coro = [&]()->inline_task<void> {
        ...
        while (true) {
            ...
            auto [err, n] = co_await sock.async_receive_from(net::buffer(buf, sizeof(buf)), ep, asio2exec::use_sender);
            if (err) {
                std::cerr << "Receive error: " << err.message() << '\n';
                co_return;
            }
            std::cout << "Received " << n << " bytes from " << ep.address().to_string() << ':' << ep.port() << '\n';
    ...
    auto request_coro = [&]()->inline_task<void> {
        ...
        auto [resp, from] = co_await client->request(server_ep, std::move(req), std::chrono::milliseconds(1000), 3);
        if (!resp) {
            std::cerr << "Request error\n";
            co_return;
        }
        std::cout << "Received response from " << from.address() << ':' << from.port() << '\n';
    ...
    auto work = stdexec::when_all(
        recv_coro(),
        request_coro() | stdexec::let_value([] { return stdexec::just_stopped(); })
    );
