...
struct message {
    ...
    struct integrity_t {
        ...
        std::span<const std::byte> message_before_integrity()const noexcept { return {_msg, _hash_value.data() - 4}; }
        std::span<const std::byte> hash_value()const noexcept { return _hash_value; }
    private:
    ...
    struct password_algorithm_t {
        ...
        password_algorithm_t(uint16_t algo_type, std::span<const std::byte> param):
            _algo{ algo_type },
            _value(param.begin(), param.end())
        {
            if (_value.size() > 256)
                throw std::runtime_error{ "password algorithm's parameter size greater than 256" };
        }

        uint16_t algo()const noexcept { return _algo; }
        std::span<const std::byte> parameter()const noexcept { return {_value.data(), _value.size()}; }
        std::string get_hmac_key(std::string_view username, std::string_view realm, std::string_view password);
    private:
        uint16_t _algo{};
    ...
    bool parse(const void* data, std::size_t length, std::size_t* offset = nullptr)noexcept;
    ...
    void add_message_integrity(integrity_t::algo_t algo) { _integrity_algos.push_back(algo); }
