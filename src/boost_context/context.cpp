...
struct context_initializer {
    ...
    void deinitialize() { delete active; }
};
...
void context::resume() {
    ...
    std::move(_c).resume_with([prev](boost::context::fiber &&c) noexcept {
        prev->_c = std::move(c);
...
void context::yield() {
    ...
    std::move(caller->_c)
        .resume_with([current](boost::context::fiber &&c) noexcept {
