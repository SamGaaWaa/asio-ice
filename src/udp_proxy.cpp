...
std::shared_ptr<udp_connection>
udp_proxy::connect(const net::ip::udp::endpoint &peer, std::size_t buf_size) {
...
ice::inline_task<void> udp_proxy::read_loop(std::shared_ptr<udp_proxy> self) {
    ...
    while (true) {
        ...
        pkg.resize(_mtu);
        auto [err, n] = co_await _sock.async_receive_from(
            net::buffer(pkg.data(), pkg.size()), ep, asio2exec::use_sender);
        if (err) {
