...
std::optional<SackChunk> SackChunk::Parse(std::span<const uint8_t> data) {
  ...
  if (reader->variable_data_size() != nbr_of_gap_blocks * kGapAckBlockSize +
    ...
    SCTP_DLOG_WARNING() << "Invalid number of gap blocks or duplicate TSNs";
    return std::nullopt;
  }
  ...
  SCTP_DCHECK(offset == reader->variable_data_size());
...
void SackChunk::SerializeTo(std::vector<uint8_t>& out) const {
  ...
  SCTP_DCHECK(offset == variable_size);
}
...
std::string SackChunk::ToString() const {
  ...
  if (!duplicate_tsns_.empty()) {
    sb << ", dup_tsns="
       << dcsctp::StrJoin(duplicate_tsns(), ",",
                          [](std::ostringstream& sb, TSN tsn) { sb << *tsn; });
  }
