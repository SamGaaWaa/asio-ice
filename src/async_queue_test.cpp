...
template<class T>
using coro_task = exec::__task::basic_task<T, exec::__task::__raw_task_context<T>>;

uint64_t asio_channel_test(int times) {
    using namespace ice;

    net::io_context ctx;
    net::experimental::channel<void(ice::error_code, int) > ch{ ctx, buffer_max_size };
    ...
    auto reader = [&]()->net::awaitable<void> {
        for (int i = 0; i < times; ++i) {
            int ret;
            if (!ch.try_receive([&](ice::error_code, int x) {
                ret = x;
            }))
...
uint64_t async_queue_test(int times) {
    ...
    auto writer = [&]()->coro_task<void> {
        for (int i = 1; i <= times; ++i) {
            ...
            q.push(std::make_tuple(ice::error_code{}, i), [](auto& container) {
                container.pop_front();
            });
        }
        q.close();
...
uint64_t async_queue_with_circular_buffer_test(int times) {
    ...
    net::io_context ctx;
    asio2exec::scheduler_t sched{ ctx };
    async_queue<std::tuple<ice::error_code, int>, boost::circular_buffer<std::tuple<ice::error_code, int>>> q(buffer_max_size);
