...
template <class T> struct property {
    template <class... Args>
    explicit property(Args &&...args) : _val(std::forward<Args>(args)...) {}

    property(const property &other) : _val(other._val) {}

    property &operator=(const property &other) { return assign(other._val); }

    property &operator=(const T &x) { return assign(x); }

    property(property &&) = delete;
    property &operator=(property &&) = delete;

    operator const T &() const noexcept { return _val; }
    ...
    auto update_time() const noexcept { return _update_time; }

    auto on_change() noexcept { return _notifier.get_future(); }
