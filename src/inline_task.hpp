...
namespace __detail{
    namespace __coro_alloc{
        ...
        class coro_alloc{
            ...
            static dealloc_fn *__dealloc_address(std::uintptr_t ptr, std::size_t size)noexcept{
                constexpr auto dealloc_fn_align = alignof(dealloc_fn);
                const auto aligned = (ptr + size + dealloc_fn_align - 1) / dealloc_fn_align * dealloc_fn_align;
                return std::launder(reinterpret_cast<dealloc_fn*>(aligned));
            }

            template<class Alloc>
            static Alloc *__alloc_address(std::uintptr_t ptr, std::size_t size)noexcept{
                constexpr auto alloc_align = alignof(Alloc);
                const auto d_fn = reinterpret_cast<std::uintptr_t>(__dealloc_address(ptr, size)) + sizeof(dealloc_fn);
                const auto aligned = (d_fn + alloc_align - 1) / alloc_align * alloc_align;
            ...
            //         }
            //     }();
            //     return size + alloc_align + alignof(dealloc_fn) + alloc_size + sizeof(dealloc_fn);
            // }

            template<class Alloc>
            static std::size_t __alloc_size(std::size_t size)noexcept{
                return reinterpret_cast<std::uintptr_t>(__alloc_address<Alloc>(0, size)) + sizeof(Alloc);
            }
            ...
            static void __deallocator(void* ptr, std::size_t size)noexcept{
                ...
                if constexpr(is_stateless<Alloc>){
                    ...
                    Alloc* a_ptr = __alloc_address<Alloc>(reinterpret_cast<std::uintptr_t>(ptr), size);
            ...
            static void* __allocate(const Alloc& a, std::size_t size)noexcept{
                using rebind_a_t = typename std::allocator_traits<Alloc>::template rebind_alloc<block_t>;
                using rebind_attr_t = typename std::allocator_traits<Alloc>::template rebind_traits<block_t>;
                ...
                auto d_fn_ptr = __dealloc_address(reinterpret_cast<std::uintptr_t>(p), size);
                ...
                *d_fn_ptr= d_fn;
                if constexpr(!is_stateless<rebind_a_t>){
                    auto *a_ptr = __alloc_address<rebind_a_t>(reinterpret_cast<std::uintptr_t>(p), size);
        ...
        public:
            ...
            void *operator new(std::size_t size, std::allocator_arg_t, const Alloc& a, const Args& ... args){
                return __allocate(a, size);
            }

            template<class This, class Alloc, class ...Args>
                requires std::is_nothrow_move_constructible_v<Alloc>
            void *operator new(std::size_t size, const This&, std::allocator_arg_t, const Alloc& a, const Args& ... args){
                return __allocate(a, size);
            }

            void operator delete(void *ptr, std::size_t size)noexcept{
                auto d_fn_ptr = __dealloc_address(reinterpret_cast<std::uintptr_t>(ptr), size);
                (*d_fn_ptr)(ptr, size);
    ...
    // , __coro_alloc::coro_alloc
    {
        ...
        bool stop_possible()const noexcept{
            return _stop_token.stop_possible();
        }
        ...
        struct env_t{
            ...
            friend auto tag_invoke(stdexec::get_stop_token_t, const env_t& env)noexcept
                -> stdexec::inplace_stop_token
            {
                return env.promise->get_token();
            }

            // friend auto tag_invoke(stdexec::get_allocator_t, const env_t& env)noexcept
            //     -> std::pmr::polymorphic_allocator<>
            // {
        ...
        friend env_t tag_invoke(stdexec::get_env_t, const promise_base& promise)noexcept{
            return {&promise};
        }
    private:
    ...
    struct __any_stop_callback_t{
        ...
        __any_stop_callback_t(const StopToken& parent_token, stdexec::inplace_stop_source& self){
            if constexpr(stdexec::unstoppable_token<std::decay_t<StopToken>>){
                ...
                using callback_t = typename std::decay_t<StopToken>:: template callback_type<forward_stop_request_t>;
                ...
                if constexpr(sizeof(callback_t) <= sizeof(_storage)){
                    ...
                    new (_callback) callback_t(parent_token, forward_stop_request_t{&self});
                }else{
                    _callback = new callback_t(parent_token, forward_stop_request_t{&self});
                }
    ...
    struct stop_callback_t{
        ...
        using callback_t = typename stdexec::inplace_stop_token::callback_type<forward_stop_request_t>;

        template<class StopToken>
        stop_callback_t(const StopToken& parent_token, stdexec::inplace_stop_source& self_source){
            using stop_token_type = std::decay_t<StopToken>;
            if constexpr(stdexec::unstoppable_token<stop_token_type>)
                return;
            else if constexpr(std::is_same_v<stop_token_type, stdexec::inplace_stop_token>){
                _callback.emplace<1>(
                    parent_token,
    ...
    struct task_awaitable_base{
            ...
            -> std::coroutine_handle<>
        {
            ...
            if constexpr(requires { stdexec::get_env(parent.promise()); 
                                    stdexec::get_stop_token(stdexec::get_env(parent.promise())); 
                                    } && !stdexec::unstoppable_token<stdexec::stop_token_of_t<stdexec::env_of_t<ParentPromise>>>){
    ...
    struct task_awaitable: __detail::task_awaitable_base<promise_type>{
        ...
        T await_resume(){
            ...
            struct __guard{
                ...
                __guard(std::coroutine_handle<promise_type> *h)noexcept: _h{h} {}
                ~__guard(){
                    std::exchange(*_h, {}).destroy();
                }
            };
            __guard on_exit(&this->h);
            if(this->h.promise().error)[[unlikely]]
                std::rethrow_exception(std::exchange(this->h.promise().error, nullptr));
