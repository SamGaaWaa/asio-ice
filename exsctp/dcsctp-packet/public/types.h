...
#include "common/units/time_delta.h"
#include "common/strong_alias.h"

namespace dcsctp {

// Stream Identifier
using StreamID = dcsctp::utils::StrongAlias<class StreamIDTag, uint16_t>;

// Payload Protocol Identifier (PPID)
using PPID = dcsctp::utils::StrongAlias<class PPIDTag, uint32_t>;

// Timeout Identifier
using TimeoutID = dcsctp::utils::StrongAlias<class TimeoutTag, uint64_t>;

// Indicates if a message is allowed to be received out-of-order compared to
// other messages on the same stream.
using IsUnordered = dcsctp::utils::StrongAlias<class IsUnorderedTag, bool>;
...
using StreamPriority = dcsctp::utils::StrongAlias<class StreamPriorityTag, uint16_t>;

// Duration, as milliseconds. Overflows after 24 days.
class DurationMs : public dcsctp::utils::StrongAlias<class DurationMsTag, int32_t> {
 public:
  constexpr explicit DurationMs(const UnderlyingType& v)
      : dcsctp::utils::StrongAlias<class DurationMsTag, int32_t>(v) {}

  constexpr explicit DurationMs(dcsctp::utils::TimeDelta v)
      : dcsctp::utils::StrongAlias<class DurationMsTag, int32_t>(
  ...
  constexpr dcsctp::utils::TimeDelta ToTimeDelta() const {
    return *this == DurationMs::InfiniteDuration()
               ? dcsctp::utils::TimeDelta::PlusInfinity()
               : dcsctp::utils::TimeDelta::Millis(value_);
  }
};
...
class TimeMs : public dcsctp::utils::StrongAlias<class TimeMsTag, int64_t> {
 public:
  constexpr explicit TimeMs(const UnderlyingType& v)
      : dcsctp::utils::StrongAlias<class TimeMsTag, int64_t>(v) {}
...
class MaxRetransmits
    : public dcsctp::utils::StrongAlias<class MaxRetransmitsTag, uint16_t> {
 public:
  constexpr explicit MaxRetransmits(const UnderlyingType& v)
      : dcsctp::utils::StrongAlias<class MaxRetransmitsTag, uint16_t>(v) {}
...
// set.
class LifecycleId : public dcsctp::utils::StrongAlias<class LifecycleIdTag, uint64_t> {
 public:
  constexpr explicit LifecycleId(const UnderlyingType& v)
      : dcsctp::utils::StrongAlias<class LifecycleIdTag, uint64_t>(v) {}
...
class ZeroChecksumAlternateErrorDetectionMethod
    : public dcsctp::utils::StrongAlias<
 ...
 public:
  constexpr explicit ZeroChecksumAlternateErrorDetectionMethod(
      ...
      : dcsctp::utils::StrongAlias<class ZeroChecksumAlternateErrorDetectionMethodTag,
                            uint32_t>(v) {}
