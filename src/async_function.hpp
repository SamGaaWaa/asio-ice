...
template <class Ret, class ...Args>
struct async_function<Ret(Args...)> {
    async_function() = default;

    template <class Func>
    explicit async_function(Func&& f) {
        if constexpr (stdexec::sender<std::invoke_result_t<Func, Args...>>) {
            _func.template emplace<1>(wrap<std::decay_t<Func>>{std::forward<Func>(f)});
        } else {
    ...
    template <class Func>
    async_function& operator=(Func&& f) {
        if constexpr (stdexec::sender<std::invoke_result_t<Func, Args...>>) {
            _func.template emplace<1>(wrap<std::decay_t<Func>>{std::forward<Func>(f)});
        } else {
            _func.template emplace<0>(std::forward<Func>(f));
    ...
    bool is_async() const noexcept {
        return _func.index() == 1 &&
                std::get<1>(_func) != nullptr;
    }

    bool is_sync() const noexcept {
        return _func.index() == 0 &&
                std::get<0>(_func) != nullptr;
    }

    template <class ...Args1>
    Ret invoke_sync(Args1&& ...args) {
        return std::get<0>(_func)(std::forward<Args1>(args)...);
    }

    template <class ...Args1>
    ice::task<Ret> invoke_async(Args1&& ...args) {
        return std::get<1>(_func)(std::forward<Args1>(args)...);
    }

    template <class ...Args1>
    auto invoke(Args1&& ...args) {
        using sync_sender_t = std::decay_t<decltype(stdexec::just(invoke_sync(std::forward<Args1>(args)...)))>;
        using return_type = exec::variant_sender<sync_sender_t, ice::task<Ret>>;
        if (_func.index() == 0)
            return return_type{stdexec::just(invoke_sync(std::forward<Args1>(args)...))};
        return return_type{invoke_async(std::forward<Args1>(args)...)};
    }

    template <class ...Args1>
    auto operator()(Args1&& ...args) {
...
private:
    ...
    template <class SenderFunc>
        requires std::is_same_v<std::invoke_result_t<SenderFunc, Args...>, ice::task<Ret>>
    struct wrap<SenderFunc> {
        SenderFunc func;
        template <class ...Args1>
        ice::task<Ret> operator()(Args1&& ...args) {
...
template <class ...Args>
struct async_function<void(Args...)> {
    async_function() = default;
    ...
    template <class ...Args1>
    void invoke_sync(Args1&& ...args) {
        std::get<0>(_func)(std::forward<Args1>(args)...);
    }

    template <class ...Args1>
    ice::task<void> invoke_async(Args1&& ...args) {
        return std::get<1>(_func)(std::forward<Args1>(args)...);
    }
    ...
    auto invoke(Args1&& ...args) {
        ...
        using return_type = exec::variant_sender<sync_sender_t, ice::task<void>>;
        if (_func.index() == 0) {
...
private:
    ...
    template <class SenderFunc>
        requires std::is_same_v<std::invoke_result_t<SenderFunc, Args...>, ice::task<void>>
    struct wrap<SenderFunc> {
        SenderFunc func;
        template <class ...Args1>
        ice::task<void> operator()(Args1&& ...args) {
            return func(std::forward<Args1>(args)...);
        }
    };
