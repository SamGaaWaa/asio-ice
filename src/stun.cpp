...
static_assert(sizeof(header_t) == 20, "STUN message header size must be 20 bytes");
...
static_assert(sizeof(attr_t) == 4, "STUN attribute header size must be 4 bytes");
...
static_assert(sizeof(value_error_code_t) == 4, "value_error_code_t size must be 4 bytes");
...
static_assert(sizeof(value_channel_number_t) == 4, "value_channel_number_t size must be 4 bytes");
...
static_assert(sizeof(value_even_port_t) == 1, "value_even_port_t size must be 1 bytes");
...
static_assert(sizeof(value_requested_transport_t) == 4, "value_requested_transport_t size must be 4 bytes");
...
static_assert(sizeof(value_password_algorithm_t) == 4, "value_password_algorithm_t size must be 4 bytes");
...
struct value_mapped_address_t {
	...
	uint8_t operator[](std::size_t i)const noexcept {
		return address()[i];
	}

	uint8_t& operator[](std::size_t i)noexcept {
		return address()[i];
	}
};
...
struct attr_iterator_t {
	...
	attr_t& operator*()const noexcept {
		return *_current;
	}

	const attr_t* operator->()const noexcept {
		return _current;
	}

	attr_t* operator->()noexcept {
		return _current;
	}
	...
	attr_iterator_t& operator++()noexcept {
		...
		if (len & 0x3)
			len += 4 - (len & 0x3);
		_current = reinterpret_cast<attr_t*>(reinterpret_cast<uint8_t*>(_current) + len);
		return *this;
	}
	...
	const uint8_t* data()const noexcept { return reinterpret_cast<const uint8_t*>(_current); }
private:
	attr_t* _current;
};
...
struct const_attr_iterator_t {
	...
	const_attr_iterator_t& operator++()noexcept {
		...
		if (len & 0x3)
			len += 4 - (len & 0x3);
		_current = reinterpret_cast<const attr_t*>(reinterpret_cast<const uint8_t*>(_current) + len);
		return *this;
	}
...
static_assert(std::sentinel_for<attr_iterator_sentinel_t, const_attr_iterator_t>);

static int parse_address(const void* data, std::size_t buf_size, message::endpoint_t& address)
{
	uint8_t pad = 0;
	uint8_t protocol = 0;
	uint16_t port = 0;
	std::error_code ec;
	if(binary::unpack<"!BBH">(data, buf_size, ec, pad, protocol, port); ec || pad != 0)
		return -1;
...
static int parse_xor_address(
	...
	if (buf_size < 4)
		return -1;
    const value_mapped_address_t *mapped_address = reinterpret_cast<const value_mapped_address_t*>(data);
	...
	if (binary::ntoh<uint8_t>(mapped_address->family) == 1) {
		...
		if(4 + 4 > buf_size)
            return -1;
		address.port = binary::ntoh<uint16_t>(mapped_address->port ^ *reinterpret_cast<const uint16_t*>(&header->magic));
		address.address = net::ip::address_v4(
			binary::ntoh<uint32_t>(*reinterpret_cast<const uint32_t*>(mapped_address->address()) ^ header->magic)
		);
		return 4 + 4;
	}
		...
		net::ip::address_v6::bytes_type bytes;
		address.port = binary::ntoh<uint16_t>(mapped_address->port ^ *reinterpret_cast<const uint16_t*>(&header->magic));
		const uint32_t* v32 = reinterpret_cast<const uint32_t*>(mapped_address->address());
...
bool message::integrity_t::verify(std::string_view key) {
	ICE_IN_DEBUG{ std::cout << "Check message integrity with key: " << key << '\n'; };
	uint16_t tmp_len = *reinterpret_cast<const uint16_t*>(_msg + 2);
	binary::write_big<uint16_t>(const_cast<std::byte*>(_msg) + 2, _hash_value.data() - _msg - sizeof(header_t) + _hash_value.size());
	::HMAC_CTX* ctx = ::HMAC_CTX_new();
	if (!ctx)
		return false;
	::HMAC_Init(ctx, key.data(), key.size(), _algo == SHA1 ? ::EVP_sha1() : ::EVP_sha256());
	::HMAC_Update(ctx, reinterpret_cast<const unsigned char*>(_msg), _hash_value.data() - _msg - 4);
	unsigned char res[32];
	unsigned int res_len;
	::HMAC_Final(ctx, res, &res_len);
	::HMAC_CTX_free(ctx);
	*reinterpret_cast<uint16_t*>(const_cast<std::byte*>(_msg) + 2) = tmp_len;
	return res_len == _hash_value.size() && std::memcmp(res, _hash_value.data(), res_len) == 0;
}

std::string message::password_algorithm_t::get_hmac_key(std::string_view username, std::string_view realm, std::string_view password)
{
	std::string input;
	input.resize_and_overwrite(username.size() + realm.size() + password.size() + 2 + 32, [&](char* p, std::size_t n)->std::size_t {
		p += 32;
		...
		*p++ = ':';
		...
		if (_algo == password_algorithm_t::SHA256) {
			key_end = ::SHA256(reinterpret_cast<const unsigned char*>(input.data() + 32), p - input.data() - 32, reinterpret_cast<unsigned char*>(input.data()));
		}
		else {
			key_end = ::MD5(reinterpret_cast<const unsigned char*>(input.data() + 32), p - input.data() - 32, reinterpret_cast<unsigned char*>(input.data()));
		}
...
bool message::parse(const void* data, std::size_t buf_size, std::size_t* offset)noexcept
{
	if (offset) {
		if (*offset >= buf_size)
			return false;
	...
	std::copy(header->transaction_id, header->transaction_id + 12, transaction_id.data());
	...
	for (; iter != end; ++iter) {
		...
		case attr_type_t::STUN_ATTR_MAPPED_ADDRESS: {
			if ((attr_len != 8 && attr_len != 20) ||
				parse_address(attr.value(), end.data() - attr.value(), this->mapped_address.emplace()) != attr_len)
				return false;
			break;
		}
		case attr_type_t::STUN_ATTR_XOR_MAPPED_ADDRESS: {
			if ((attr_len != 8 && attr_len != 20) ||
				parse_xor_address(attr.value(), end.data() - attr.value(), header, this->xor_mapped_address.emplace()) != attr_len)
				return false;
			break;
		}
		case attr_type_t::STUN_ATTR_ALTERNATE_SERVER: {
			if ((attr_len != 8 && attr_len != 20) ||
				parse_address(attr.value(), end.data() - attr.value(), this->alternate_server.emplace()) != attr_len)
				return false;
			break;
		}
		case attr_type_t::STUN_ATTR_ERROR_CODE: {
			...
			if (attr_len > sizeof(value_error_code_t) + 762)
				return false;
			if (attr_len > sizeof(value_error_code_t))
				this->error_code->reason = std::string_view{ec->reason(), static_cast<std::size_t>(attr_len - 4)};
			break;
		}
		...
		case attr_type_t::STUN_ATTR_USERNAME: {
			if (attr_len > 513)
				return false;
			this->username = std::string_view{ reinterpret_cast<const char*>(attr.value()), attr_len };
			break;
		}
		case attr_type_t::STUN_ATTR_MESSAGE_INTEGRITY: {
			...
			integrity._hash_value = std::span<const std::byte>{ reinterpret_cast<const std::byte*>(attr.value()), attr_len };
		...
		case attr_type_t::STUN_ATTR_FINGERPRINT: {
			...
			if (attr_len != 4)
				return false;
			fingerprint = true;
			uint16_t tmp_len = binary::hton<uint16_t>(iter.data() - begin - sizeof(header_t) + 4 + 4);
			boost::crc_32_type crc;
			crc.process_bytes(begin, 2);
			crc.process_bytes(&tmp_len, 2);
			crc.process_bytes(begin + 4, iter.data() - begin - 4);
			if ((crc.checksum() ^ 0x5354554e) != binary::read_big<uint32_t>(attr.value())) {
		...
		case attr_type_t::STUN_ATTR_REALM: {
			if (attr_len > 763 || attr_len == 0)
				return false;
			this->realm = std::string_view{ reinterpret_cast<const char*>(attr.value()), attr_len };
			break;
		}
		case attr_type_t::STUN_ATTR_NONCE: {
			if (attr_len > 763 || attr_len == 0)
				return false;
			this->nonce = std::string_view{ reinterpret_cast<const char*>(attr.value()), attr_len };
			break;
		}
		...
		case attr_type_t::STUN_ATTR_PASSWORD_ALGORITHMS: {
			...
			for (; pwd_it != pwd_end; ++pwd_it) {
				const auto *value_pwd_algo = reinterpret_cast<const value_password_algorithm_t*>(pwd_it.data());
				const uint16_t pwd_algo_type = binary::ntoh<uint16_t>(value_pwd_algo->algorithm);
				const auto pwd_algo_len = binary::ntoh<uint16_t>(value_pwd_algo->parameters_length);
				if (pwd_algo_len > 256)
					return false;
				if (pwd_algo_type != password_algorithm_t::MD5 && pwd_algo_type != password_algorithm_t::SHA256) {
					ICE_IN_DEBUG{ std::cerr << "Unknown password algorithm:" << pwd_algo_type << '\n'; }
					continue;
				}
				this->password_algorithms.emplace_back(
					pwd_algo_type,
					std::span<const std::byte>{reinterpret_cast<const std::byte*>(value_pwd_algo->parameters()), pwd_algo_len}
				);
			}
		...
		case attr_type_t::STUN_ATTR_SOFTWARE: {
			if (attr_len > 763)
				return false;
			this->software = std::string_view{ reinterpret_cast<const char*>(attr.value()), attr_len };
			break;
		}
		...
		case attr_type_t::STUN_ATTR_ICE_CONTROLLING: {
			if (attr_len != 8)
				return false;
			this->ice_controlling = (static_cast<uint64_t>(binary::read_big<uint32_t>(attr.value())) << 32) | binary::read_big<uint32_t>(attr.value() + 4);
			break;
		}
		case attr_type_t::STUN_ATTR_ICE_CONTROLLED: {
			...
			this->ice_controlled = (static_cast<uint64_t>(binary::ntoh<uint32_t>(v32[0])) << 32) | binary::ntoh<uint32_t>(v32[1]);
			break;
		}
		case attr_type_t::STUN_ATTR_CHANNEL_NUMBER: {
			if (attr_len != sizeof(value_channel_number_t))
				return false;
			const auto* channel_num = reinterpret_cast<const value_channel_number_t*>(attr.value());
			if (channel_num->reserved != 0)
				return false;
			this->channel_number = binary::ntoh<uint16_t>(channel_num->channel_number);
			break;
		}
		case attr_type_t::STUN_ATTR_LIFETIME: {
			if (attr_len != 4)
				return false;
			this->lifetime = binary::ntoh<uint32_t>(*reinterpret_cast<const uint32_t*>(attr.value()));
			break;
		}
		case attr_type_t::STUN_ATTR_XOR_PEER_ADDRESS: {
			if ((attr_len != 8 && attr_len != 20) ||
				parse_xor_address(attr.value(), end.data() - attr.value(), header, this->xor_peer_address.emplace()) != attr_len)
				return false;
			break;
		}
		case attr_type_t::STUN_ATTR_XOR_RELAYED_ADDRESS: {
			if ((attr_len != 8 && attr_len != 20) ||
				parse_xor_address(attr.value(), end.data() - attr.value(), header, this->xor_relayed_address.emplace()) != attr_len)
				return false;
			break;
		}
		...
		case attr_type_t::STUN_ATTR_EVEN_PORT: {
			if (attr_len < 1)
				return false;
			const value_even_port_t* e = reinterpret_cast<const value_even_port_t*>(attr.value());
			if (e->reserved != 0)
				return false;
		...
		case attr_type_t::STUN_ATTR_REQUESTED_TRANSPORT: {
			if (attr_len != sizeof(value_requested_transport_t))
				return false;
			const auto* r = reinterpret_cast<const value_requested_transport_t*>(attr_len);
		...
		case attr_type_t::STUN_ATTR_RESERVATION_TOKEN: {
			if (attr_len != 8)
				return false;
			const uint32_t* v32 = reinterpret_cast<const uint32_t*>(attr.value());
			this->reservation_token = (static_cast<uint64_t>(binary::ntoh<uint32_t>(v32[0])) << 32) | binary::ntoh<uint32_t>(v32[1]);
			break;
		}
		default:
			// Ignore
...
static int write_address(void* data, std::size_t buf_size, const message::endpoint_t& address)
{
	if (buf_size < sizeof(value_mapped_address_t))
		return -1;
	value_mapped_address_t* mapped = reinterpret_cast<value_mapped_address_t*>(data);
	...
	if (address.address.is_v4()) {
		if (4 + 4 > buf_size)
			return -1;
		mapped->family = 1;
		binary::write_big<uint32_t>(mapped->address(), address.address.to_v4().to_uint());
		return 4 + 4;
	}
...
static int write_xor_address(
	...
	if (address.address.is_v4()) {
		...
		*value = header->magic ^ binary::hton<uint32_t>(address.address.to_v4().to_uint());
		return 4 + 4;
	}
	else if (address.address.is_v6()) {
		if (4 + 16 > buf_size)
...
{
	int len = write_header(buf, length, this->cls, this->method, this->transaction_id.data());
	if (len < 0)
		return -1;
	header_t* header = reinterpret_cast<header_t*>(buf);
	...
	if (this->error_code) {
		if (iter == end)
			goto overflow;
		uint16_t attr_size = sizeof(value_error_code_t) + this->error_code->reason.size();
		...
		value_error_code_t* ec = reinterpret_cast<value_error_code_t*>(iter.data());
		ec->reserved = 0;
	...
	if (this->mapped_address) {
		...
		auto attr_size = write_address(iter->value(), buf_end - iter->value(), *this->mapped_address);
		if (attr_size < 0)
			goto overflow;
	...
	if (this->xor_mapped_address) {
		if (iter == end)
			goto overflow;
		iter->type = binary::hton<uint16_t>(attr_type_t::STUN_ATTR_XOR_MAPPED_ADDRESS);
		auto attr_size = write_xor_address(iter->value(), buf_end - iter->value(), header, *this->xor_mapped_address);
		if (attr_size < 0)
			goto overflow;
		iter->length = binary::hton<uint16_t>(static_cast<uint16_t>(attr_size));
		++iter;
	...
	if (this->xor_peer_address) {
		if (iter == end)
			goto overflow;
		iter->type = binary::hton<uint16_t>(attr_type_t::STUN_ATTR_XOR_PEER_ADDRESS);
		auto attr_size = write_xor_address(iter->value(), buf_end - iter->value(), header, *this->xor_peer_address);
		if (attr_size < 0)
			goto overflow;
		iter->length = binary::hton<uint16_t>(static_cast<uint16_t>(attr_size));
		++iter;
	...
	if (this->xor_relayed_address) {
		if (iter == end)
			goto overflow;
		iter->type = binary::hton<uint16_t>(attr_type_t::STUN_ATTR_XOR_RELAYED_ADDRESS);
		auto attr_size = write_xor_address(iter->value(), buf_end - iter->value(), header, *this->xor_relayed_address);
		if (attr_size < 0)
			goto overflow;
		iter->length = binary::hton<uint16_t>(static_cast<uint16_t>(attr_size));
		++iter;
	...
	if (this->requested_transport) {
		...
		if (iter->value() + sizeof(value_requested_transport_t) > buf_end)
			goto overflow;
		iter->type = binary::hton<uint16_t>(attr_type_t::STUN_ATTR_REQUESTED_TRANSPORT);
	...
	if (this->reservation_token) {
		...
		if (iter->value() + 8 > buf_end)
			goto overflow;
		iter->type = binary::hton<uint16_t>(attr_type_t::STUN_ATTR_RESERVATION_TOKEN);
		iter->length = binary::hton<uint16_t>(8);
	...
	for (const auto& algo : _integrity_algos) {
		...
		case integrity_t::SHA1:
			iter->type = binary::hton<uint16_t>(attr_type_t::STUN_ATTR_MESSAGE_INTEGRITY);
			attr_size = 20;
			iter->length = binary::hton<uint16_t>(20);
			break;
		case integrity_t::SHA256:
			iter->type = binary::hton<uint16_t>(attr_type_t::STUN_ATTR_MESSAGE_INTEGRITY_SHA256);
			attr_size = 32;
			iter->length = binary::hton<uint16_t>(32);
			break;
		default:
			ICE_IN_DEBUG{ std::cerr << "Unknown integrity algorithm: " << algo << '\n'; }
			continue;
		}
		if (iter->value() + attr_size > buf_end)
			goto overflow;
		header->length = binary::hton<uint16_t>((iter.data() - buf_begin) - sizeof(header_t) + 4 + attr_size);
		::HMAC_CTX* ctx = ::HMAC_CTX_new();
		if (!ctx)
			continue;
		::HMAC_Init(ctx, _hmac_key.data(), (int)_hmac_key.size(), algo == integrity_t::SHA1 ? ::EVP_sha1() : ::EVP_sha256());
		::HMAC_Update(ctx, (const unsigned char*)buf_begin, iter.data() - buf_begin);
		...
		integrity._hash_value = std::span<const std::byte>(reinterpret_cast<const std::byte*>(iter->value()), attr_size);
	...
	if (_use_fingerprint) {
		...
		header->length = binary::hton<uint16_t>((iter.data() - buf_begin) - sizeof(header_t) + 4 + 4);
		boost::crc_32_type crc;
	...
	binary::write_big<uint16_t>(&header->length, iter.data() - buf_begin - sizeof(header_t));
...
static nlohmann::json to_json(const message::password_algorithm_t& pa)noexcept {
	nlohmann::json obj;
...
std::string message::to_string() {
	...
	obj["transaction_id"] = base64::encode(transaction_id.data(), transaction_id.size());
	...
	if (this->userhash) {
		...
		userhash_obj["value"] = base64::encode(this->userhash->data(), USERHASH_SIZE);
	...
	for (const auto& integrity : this->integrities) {
		...
		integrity_obj["type"] = std::string{ "integrity_" } + algo_name(integrity.algo());
