...
std::string_view ToString(ReconfigurationResponseParameter::Result result) {
  switch (result) {
...
ReconfigurationResponseParameter::Parse(std::span<const uint8_t> data) {
  ...
  switch (result_nbr) {
    ...
    case 6:
      ...
      break;
    default:
      SCTP_DLOG_WARNING() << "Invalid reconfig response result: "
                           << result_nbr;
      return std::nullopt;
  }
  ...
  if (reader->variable_data().empty()) {
    ...
    SCTP_DLOG_WARNING() << "Invalid parameter size";
    return std::nullopt;
  }
