...
public:
  ...
  template <bool stoppable> struct result {
    async_queue *self;
  ...
  bool is_open() const noexcept { return !_closed; }

  bool full() const noexcept { return _q.size() == _max; }

  bool empty() const noexcept { return _q.empty(); }

private:
  struct read_awaitable_base
      : public boost::intrusive::list_base_hook<
            boost::intrusive::link_mode<boost::intrusive::auto_unlink>> {
    read_awaitable_base(async_queue &self) noexcept : _self(self) {}

    read_awaitable_base(read_awaitable_base &&) = delete;

    template <class _T> void set_value(_T &&t) {
      _result.emplace(std::forward<_T>(t));
      _h.resume();
    }

    void set_stopped() noexcept { _h.resume(); }
  ...
  struct read_awaitable : read_awaitable_base {
    read_awaitable(async_queue &self) noexcept : read_awaitable_base(self) {}

    ~read_awaitable() { destroy_stop_callback(); }
    ...
    std::coroutine_handle<> await_suspend(std::coroutine_handle<Promise> h) {
      if constexpr (requires {
                    ...
                    !stdexec::unstoppable_token<
                        stdexec::stop_token_of_t<stdexec::env_of_t<Promise>>>) {
  ...
  private:
    ...
    void construct_stop_callback(Token token,
                                 std::coroutine_handle<Promise> h) {
      using stop_callback_t =
          stdexec::stop_token_of_t<stdexec::env_of_t<Promise>>::
              template callback_type<std::coroutine_handle<Promise>>;
      if constexpr (sizeof(stop_callback_t) > sizeof(_storage)) {
        ...
        std::construct_at(reinterpret_cast<stop_callback_t *>(_storage), token,
                          h);
  ...
  using readers_list_t =
      boost::intrusive::list<read_awaitable_base,
