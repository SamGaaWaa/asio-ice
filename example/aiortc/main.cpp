...
namespace asioice { namespace net = boost::asio; }
...
ice_dtls_sctp_session(net::io_context &ctx, ws_ptr ws) {
    ...
    auto hs_ec =
        co_await dgram->socket().async_handshake(
    ...
    dc_mgr.on_remote_channel(
        [&dc_mgr, &scope](std::shared_ptr<Datachannel> ch) {
            ...
            scope.spawn([](std::shared_ptr<Datachannel> ch)-> asioice::task<void> {
                while (true) {
                    ...
                    if (!msg) {
                        std::cerr << "channel \"" << ch->label() << "\" read failed\n";
                        co_return;
                    }
                    if (!msg->binary)
                        std::cout << "channel \"" << ch->label() << "\" received: " <<
                            std::string_view{(const char*)msg->data.data(), msg->data.size()} << '\n';
                }
            }(std::move(ch)));
        });
...
int main() {
    ...
    net::io_context ctx;
    exec::start_detached(
        stdexec::starts_on(asio2exec::scheduler{ctx},
                           listener(ctx)));
    ctx.run();
}
