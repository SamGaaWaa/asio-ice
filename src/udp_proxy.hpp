...
struct udp_proxy: std::enable_shared_from_this<udp_proxy> {
    ...
    udp_proxy(net::io_context& ctx, const net::ip::udp::endpoint& local, Protocol protocol, std::size_t pkg_cache_size = 16):
        _ctx(ctx),
        _local(local),
        _sock(ctx, protocol),
    ...
    std::shared_ptr<udp_connection> connect(const net::ip::udp::endpoint& peer, std::size_t buf_size = 16);

    void start();
    void stop()noexcept;

    net::ip::udp::socket& socket()noexcept {
        return _sock;
    }

    const net::ip::udp::endpoint& local()const noexcept {
        return _local;
    }
...
private:
	...
	struct peer_hasher_t {
		std::size_t operator()(const net::ip::udp::endpoint& peer)const noexcept {
			return std::hash<std::string_view>{}(
				std::string_view{ reinterpret_cast<const char*>(peer.data()), peer.size() }
			);
		}
	};

    struct peer_compare_t {
        bool operator()(const net::ip::udp::endpoint& x, const net::ip::udp::endpoint& y)const noexcept {
            return std::string_view{ reinterpret_cast<const char*>(x.data()), x.size() } <
                std::string_view{ reinterpret_cast<const char*>(y.data()), y.size() };
        }
    };

    //using connection_map_t = std::unordered_map<net::ip::udp::endpoint, udp_connection*, peer_hasher_t>;
    using connection_map_t = boost::container::flat_map<net::ip::udp::endpoint, udp_connection*, peer_compare_t>;
    ...
    std::function<bool(const net::ip::udp::endpoint&, ice::packet&)> _filter{ nullptr };
    bool _started{ false };
