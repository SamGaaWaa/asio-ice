...
bool client::dispatch(const net::ip::udp::endpoint &ep, const void *data,
                      std::size_t size) {
  assert(_running);
...
client::transaction_t::retry(std::chrono::milliseconds timeout,
  ...
  utils::scope_guard on_exit([this]() noexcept { _done_promise.set_value(); });
  net::steady_timer timer{_client->context()};
  for (std::size_t i = 0; i < max_retries; ++i) {
    auto [err, n] = co_await _client->socket().async_send_to(
        net::buffer(_buf, _buf_size), this->endpoint, asio2exec::use_sender);
    if (err) {
      ICE_IN_DEBUG {
    ...
    if (n != _buf_size) {
      ICE_IN_DEBUG {
        // std::cerr << "STUN transaction failed: sent " << n << " bytes,
        // expected " << _buf_size << "\n";
      }
      co_return;
    }
...
void client::transaction_t::run(std::chrono::milliseconds timeout,
  ...
  stdexec::start_detached(stdexec::starts_on(
      sched, stdexec::when_all(retry(timeout, max_retries, shared_from_this()),
