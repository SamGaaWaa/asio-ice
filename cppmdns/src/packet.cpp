...
__parse_name(std::string& name, const void* buf, std::size_t size, std::size_t& offset, int depth){
    if(depth > 20)
    ...
    while(iter < end){
        ...
        if(len == 0){
            if(name.empty())
                ...
                if(!__is_valid_domain_name(name)){
                    CPPMDNS_IN_DEBUG{
                        std::cerr << "invalid domain name: " << name << '\n';
                    }
                    return std::error_code{(int)error::invalid_domain_name, mdns_category()};
                }
            }
        ...
        if((len & 0b11000000) == 0b11000000){
            ...
            if(ptr >= offset){
                CPPMDNS_IN_DEBUG{
                    std::cerr << "invalid pointer: " << ptr << " >= " << offset << '\n';
                }
                return std::error_code{(int)error::invalid_format, mdns_category()};
            }
            std::size_t ptr_offset = ptr;
            const auto ec =  __parse_name(name, buf, offset, ptr_offset, depth + 1);
...
static std::error_code __parse_cname(rdata::cname& data, const void *buf, const void *ptr, std::size_t rdlength){
...
static void __parse_hinfo(rdata::hinfo& data, const void *buf, const void *ptr, std::size_t rdlength){
    // TODO
}

static void __write_hinfo(const rdata::hinfo& data, void *ptr, std::size_t rdlength)noexcept{
    // TODO
}

static std::error_code __parse_mx(rdata::mx& data, const void *buf, const void *ptr, std::size_t rdlength){
    data.preference = binary::read_big<int16_t>(ptr);
    std::size_t offset = (const char*)ptr - (const char*)buf + 2;
    const auto ec =  __parse_name(data.exchange, buf, (const char*)ptr - (const char*)buf + rdlength, offset, 1);
    if(offset != (const char*)ptr - (const char*)buf + rdlength)
        return std::error_code{(int)error::invalid_format, mdns_category()};
    return ec;
}

static std::error_code __parse_ns(rdata::ns& data, const void *buf, const void *ptr, std::size_t rdlength){
    std::size_t offset = (const char*)ptr - (const char*)buf;
    const auto ec =  __parse_name(data, buf, offset + rdlength, offset, 1);
    if(offset != (const char*)ptr - (const char*)buf + rdlength)
        return std::error_code{(int)error::invalid_format, mdns_category()};
...
static std::error_code __parse_ptr(rdata::ptr& data, const void *buf, const void *ptr, std::size_t rdlength){
    std::size_t offset = (const char*)ptr - (const char*)buf;
    const auto ec =  __parse_name(data, buf, offset + rdlength, offset, 1);
    if(offset != (const char*)ptr - (const char*)buf + rdlength)
        return std::error_code{(int)error::invalid_format, mdns_category()};
...
static std::error_code __parse_soa(rdata::soa& data, const void *buf, const void *ptr, std::size_t rdlength){
    ...
    if(ec)
        return ec;
    offset += binary::unpack<"!L3lL">((const char*)buf + offset, size - offset, ec, 
                                    data.serial,
                                    data.refresh,
                                    data.retry,
                                    data.expire,
...
static void __parse_txt(rdata::txt& data, const void *ptr, std::size_t rdlength){
    data.assign(std::string_view((const char*)ptr, rdlength));
}

static std::error_code __parse_wks(rdata::wks& data, const void *buf, const void *ptr, std::size_t rdlength){
    std::error_code ec;
...
static std::error_code __parse_srv(rdata::srv& data, const void *buf, const void *ptr, std::size_t rdlength){
    if(rdlength <= 6)
        return std::error_code{(int)error::invalid_format, mdns_category()};
    std::error_code ec;
    binary::unpack<"!3H">(ptr, rdlength, ec, data.priority, data.weight, data.port);
    if(ec)
        return ec;
...
static std::error_code __parse_record(record_t& record, const void *buf, std::size_t size, std::size_t& offset){
    auto ec = __parse_name(record.name, buf, size, offset, 1);
    if(ec)
        return ec;
    uint16_t type;
    offset += binary::unpack<"!HHlH">((const char*)buf + offset, size - offset, ec, 
                            type, 
                            (uint16_t&)record.cls,
                            record.ttl,
                            record.rdlength);
    ...
    switch((record_type)type){
        ...
        case record_type::NS:{
            ec = __parse_ns(record.data.emplace<rdata::ns>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::CNAME:{
            ec = __parse_cname(record.data.emplace<rdata::cname>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::SOA:{
            ec = __parse_soa(record.data.emplace<rdata::soa>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::WKS:{
            ec = __parse_wks(record.data.emplace<rdata::wks>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::PTR:{
            ec = __parse_ptr(record.data.emplace<rdata::ptr>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::HINFO:{
            __parse_hinfo(record.data.emplace<rdata::hinfo>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::MX:{
            ec = __parse_mx(record.data.emplace<rdata::mx>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::TXT:{
            __parse_txt(record.data.emplace<rdata::txt>(), data_ptr, record.rdlength);
            break;
        }
        ...
        case record_type::SRV:{
            ec = __parse_srv(record.data.emplace<rdata::srv>(), buf, data_ptr, record.rdlength);
            break;
        }
        case record_type::UknownType:
        default:{
            CPPMDNS_IN_DEBUG{
                std::cout << "Parse record Unknown record type: " << (uint16_t)record.type() << '\n';
            }
        }
    }
    if(ec)
...
std::expected<message_t, std::error_code> message_t::parse(const void* buf, std::size_t size){
    if(size < 12)
        return std::unexpected(std::error_code{(int)error::out_of_range, mdns_category()});
    message_t msg;
    std::error_code ec;
    binary::unpack<"!H2x4H">(buf, size, ec, 
                            msg.header.id, 
                            msg.header.questions_count,
    ...
    msg.header.flags = std::bit_cast<header_t::flags_t>(((const uint16_t*)buf)[1]);
    ...
    if(msg.header.questions_count > 0){
        ...
        for(std::size_t i = 0; i < msg.header.questions_count; ++i){
            ...
            if(ec)
                return std::unexpected(ec);
            offset += binary::unpack<"!HH">(begin + offset, size - offset, ec, (uint16_t&)question.type, (uint16_t&)question.cls);
            if(ec){
                return std::unexpected(ec);
            }
        }
...
static bool __is_valid_record(const record_t& record)noexcept{
    ...
    switch(record.type()){
        ...
        case record_type::SOA:{
            ...
            return __is_valid_domain_name(soa.mname) && __is_valid_domain_name(soa.rname);
        }
        case record_type::PTR:{
        ...
        case record_type::MX:{
            return __is_valid_domain_name(std::get<rdata::mx>(record.data).exchange);
        }
        case record_type::TXT:{
            return true;
        }
        ...
        default:{
            CPPMDNS_IN_DEBUG{
                std::cerr << "__is_valid_record Unknown record type: " << static_cast<std::uint16_t>(record.type()) << std::endl;
            }
            return false;
        }
    }
...
static std::error_code __verify_message(const message_t& msg){
    if(!msg.questions.empty() && std::ranges::any_of(msg.questions, [](const auto& question){
       return !__is_valid_domain_name(question.name);
    }))
        return std::error_code{(int)error::invalid_domain_name, mdns_category()};
    if(!msg.answers.empty() && std::ranges::any_of(msg.answers, std::not_fn(__is_valid_record)))
        return std::error_code{(int)error::invalid_domain_name, mdns_category()};
    if(!msg.authorities.empty() && std::ranges::any_of(msg.authorities, std::not_fn(__is_valid_record)))
        return std::error_code{(int)error::invalid_domain_name, mdns_category()};
    if(!msg.additionals.empty() && std::ranges::any_of(msg.additionals, std::not_fn(__is_valid_record)))
        return std::error_code{(int)error::invalid_domain_name, mdns_category()};
    return {};
}

class name_compressor {
	static constexpr auto __compare = [](const auto& x, const auto& y)noexcept {
...
public:
	...
	void finally()noexcept {
		if (_name_pool.empty()) return;
		std::ranges::sort(_name_pool, __compare);
		_name_pool.erase(std::unique(_name_pool.begin(), _name_pool.end()), _name_pool.end());
		_name_offset.resize(_name_pool.size(), -1);
		_tmp.reserve(_name_pool.back().size());
	}

	bool write_name(void* buf, std::size_t size, const std::string_view& name, std::size_t& offset)noexcept {
		...
		const auto it_tmp = std::ranges::lower_bound(_name_pool, labels, __compare);
        CPPMDNS_IN_DEBUG{
		    assert(it_tmp != _name_pool.end() && std::ranges::equal(labels, *it_tmp) && "Domain name should be prepared before writing." || [&](){
                std::cout << "Name \"" << name << "\" is not found in name pool." << '\n';
                return false;
            }());
        }
		for (auto label_begin = labels.begin(); label_begin != labels.end(); ++label_begin) {
			auto it = std::lower_bound(name_pool_begin, name_pool_end, std::ranges::subrange{label_begin, labels.end()}, __compare);
			if (it == name_pool_end) {
				// write a label
			...
			if (std::ranges::equal(*it, std::ranges::subrange{ label_begin, labels.end() })) {
				...
				// write a pointer
				if (!__write_pointer(buf, size, (uint16_t)_name_offset[idx], offset))
					return false;
				_name_offset[std::distance(_name_pool.begin(), it_tmp)] = offset_tmp;
				return true;
			}
			// write a label
			if (!__write_label(buf, size, *label_begin, offset))
		...
		if (offset + 1 > size)
			return false;
		_name_offset[std::distance(_name_pool.begin(), it_tmp)] = (int)offset_tmp;
		*((uint8_t*)buf + offset) = 0;
		++offset;
		return true;
	}
private:
	static bool __write_label(void* buf, std::size_t size, const std::string_view& label, std::size_t& offset)noexcept {
	...
	static bool __write_pointer(void* buf, std::size_t size, uint16_t pointer, std::size_t& offset)noexcept {
		if (offset + 2 > size)
			return false;
...
static void __add_rdata_to_string_pool(name_compressor& pool, const record_t& record){
    switch(record.type()){
        case record_type::NS:{
...
static void __add_record_to_string_pool(name_compressor& pool, const record_t& record){
    pool.prepare(record.name);
    __add_rdata_to_string_pool(pool, record);
}

static std::error_code 
__write_header(void* buf, std::size_t size, const message_t& msg)noexcept{
    if(size < 12)
        return std::error_code{(int)error::out_of_range, mdns_category()};
    std::error_code ec;
    binary::pack<"!H2x4H">(buf, size, ec,
                            msg.header.id,
                            msg.header.questions_count,
                            msg.header.answers_count,
                            msg.header.authorities_count,
...
static void __sort_questions(std::vector<question_t>& questions)noexcept{
    std::sort(questions.begin(), questions.end(), [](const question_t& a, const question_t& b)noexcept{
        return std::ranges::lexicographical_compare(std::views::reverse(a.name), std::views::reverse(b.name));
    });
}
...
static void __sort_records(std::vector<record_t>& records){
    std::ranges::sort(records, [](const record_t& a, const record_t& b)noexcept{
        return std::ranges::lexicographical_compare(std::views::reverse(__get_rdata_string(a)), std::views::reverse(__get_rdata_string(b)));
    });
    std::ranges::stable_sort(records, [](const record_t& a, const record_t& b)noexcept{
        return std::ranges::lexicographical_compare(std::views::reverse(a.name), std::views::reverse(b.name));
    });
}
...
static std::error_code
__write_record(void* buf, std::size_t size, const record_t& record, std::size_t& offset, name_compressor& compressor)noexcept{
    ...
    switch(record.type()){
        ...
        case record_type::NS:{
            if(!compressor.write_name(buf, size, std::get<rdata::ns>(record.data), offset))
                return std::error_code{(int)error::out_of_range, mdns_category()};
            break;
        }
        case record_type::CNAME:{
            if(!compressor.write_name(buf, size, std::get<rdata::cname>(record.data), offset))
                return std::error_code{(int)error::out_of_range, mdns_category()};
            break;
        }
        case record_type::SOA:{
        ...
        case record_type::WKS:{
            ...
            std::error_code ec;
            binary::pack<"!LB">(begin + rdata_offset, size - rdata_offset, ec, wks.address, wks.protocol);
            if (ec)
                return ec;
            break;
        }
        case record_type::PTR:{
            if(!compressor.write_name(buf, size, std::get<rdata::ptr>(record.data), offset))
                return std::error_code{(int)error::out_of_range, mdns_category()};
            break;
        }
        case record_type::HINFO:{
            // TODO:
            CPPMDNS_IN_DEBUG{
                std::cout << "Unsupported record type: " << (uint16_t)record.type() << '\n';
            }
            break;
        }
        case record_type::MX:{
        ...
        case record_type::SRV:{
            ...
            std::error_code ec;
            offset += binary::pack<"!3H">(begin + rdata_offset, size - rdata_offset, ec, srv.priority, srv.weight, srv.port);
            if (ec)
                return ec;
            // rfc2782:  Unless and until permitted by future standards action, name compression is not to be used for this field.
        ...
        default:{
            CPPMDNS_IN_DEBUG{
                std::cout << "Write record Unknown record type: " << (uint16_t)record.type() << '\n';
            }
        }
    }
    binary::write_big<uint16_t>(begin + rdata_offset - 2, uint16_t(offset - rdata_offset));
    return {};
}       

std::expected<std::size_t, std::error_code> 
message_t::write(void* buf, std::size_t size){
    ...
    compressor.reserve(questions.size() + answers.size() + authorities.size() + additionals.size());
    ...
    for(const auto& record : questions){
        ...
        if(offset + 4 > size)
            return std::unexpected(std::error_code{(int)error::out_of_range, mdns_category()});
        binary::write_big<uint16_t>((char*)buf + offset, (uint16_t)record.type);
        binary::write_big<uint16_t>((char*)buf + offset + 2, (uint16_t)record.cls);
        offset += 4;
    }
    for(const auto& record : answers){
...
static bool __fast_parse_name(const char *& iter, const char *const end)noexcept{
    while(iter < end){
...
bool message_t::possible_valid(const void *buf, std::size_t size)noexcept{
    ...
    std::error_code ec;
    binary::unpack<"!4H">((const char*)buf + 4, size - 4, ec, qdcount, arcount, nscount, addcount);
    if (ec)
        return false;
...
bool message_t::is_query()const noexcept {
    return header.flags.qr == 0;
}

bool message_t::is_response()const noexcept {
    return header.flags.qr == 1;
}
