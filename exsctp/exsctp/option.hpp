...
struct sctp_options {
  ...
  //   -4 TURN ChannelData
  // = 1191 bytes.
  static constexpr size_t MAX_SAFE_MTU_SIZE = 1191;
  ...
  uint16_t default_stream_priority = 256;
  ...
  std::chrono::milliseconds rtt_max = std::chrono::milliseconds(60'000);

  // Initial RTO value.
  std::chrono::milliseconds rto_initial = std::chrono::milliseconds(500);

  // Maximum RTO value.
  std::chrono::milliseconds rto_max = std::chrono::milliseconds(60'000);

  // Minimum RTO value. This must be larger than an expected peer delayed ack
  // timeout.
  std::chrono::milliseconds rto_min = std::chrono::milliseconds(400);

  // T1-init timeout.
  std::chrono::milliseconds t1_init_timeout = std::chrono::milliseconds(1000);

  // T1-cookie timeout.
  std::chrono::milliseconds t1_cookie_timeout = std::chrono::milliseconds(1000);

  // T2-shutdown timeout.
  std::chrono::milliseconds t2_shutdown_timeout = std::chrono::milliseconds(1000);
  ...
  std::optional<std::chrono::milliseconds> max_timer_backoff_duration = std::nullopt;

  // Hearbeat interval (on idle connections only). Set to zero to disable.
  std::chrono::milliseconds heartbeat_interval = std::chrono::milliseconds(30000);
  ...
  std::chrono::milliseconds delayed_ack_max_timeout = std::chrono::milliseconds(200);
  ...
  std::chrono::milliseconds min_rtt_variance = std::chrono::milliseconds(220);
...
struct send_options {
  ...
  std::optional<std::chrono::milliseconds> lifetime = std::nullopt;
