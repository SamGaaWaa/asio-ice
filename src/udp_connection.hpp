...
class udp_connection: 
	public std::enable_shared_from_this<udp_connection>
{
public:
	...
	void dispatch(packet pkg)noexcept {
		_q.push(std::move(pkg));
	}
	...
	auto async_send(net::const_buffer buf, CompletionToken&& token) {
		return socket().async_send_to(buf, peer(), std::forward<CompletionToken>(token));
	}
	...
	const net::ip::udp::endpoint& peer()const noexcept {
		return _peer;
	}

	udp_proxy* proxy()noexcept {
		return _proxy.get();
	}

	const udp_proxy* proxy()const noexcept {
		return _proxy.get();
	}
