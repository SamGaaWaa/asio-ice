...
get_local_addresses()
{
    ...
    ULONG out_buf_len = 0;
    DWORD ret = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, nullptr, nullptr, &out_buf_len);
    if (ret == ERROR_BUFFER_OVERFLOW) {
        adapter_addrs = static_cast<PIP_ADAPTER_ADDRESSES>(malloc(out_buf_len));
        if (!adapter_addrs) {
            return {};
        }
        ret = GetAdaptersAddresses(AF_UNSPEC, GAA_FLAG_INCLUDE_PREFIX, nullptr, adapter_addrs, &out_buf_len);
    }
    ...
    for (PIP_ADAPTER_ADDRESSES adapter = adapter_addrs; adapter != nullptr; adapter = adapter->Next) {
        for (PIP_ADAPTER_UNICAST_ADDRESS addr = adapter->FirstUnicastAddress; addr != nullptr; addr = addr->Next) {
            ...
            if (sa->sa_family == AF_INET) {
                ...
                addresses.emplace_back(net::ip::address_v6(bytes, sa_in6->sin6_scope_id));
            }
        }
    }
#else
    ...
    for (struct ifaddrs* ifa = ifaddr; ifa != nullptr; ifa = ifa->ifa_next) {
        ...
        if (family == AF_INET) {
            ...
            addresses.emplace_back(net::ip::address_v6(bytes, sa6->sin6_scope_id));
        }
    }
#endif
