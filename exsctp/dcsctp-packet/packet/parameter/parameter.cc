 ...
 */
#include "packet/parameter/parameter.h"
...
#include "packet/parameter/add_incoming_streams_request_parameter.h"
#include "packet/parameter/add_outgoing_streams_request_parameter.h"
#include "packet/parameter/forward_tsn_supported_parameter.h"
#include "packet/parameter/heartbeat_info_parameter.h"
#include "packet/parameter/incoming_ssn_reset_request_parameter.h"
#include "packet/parameter/outgoing_ssn_reset_request_parameter.h"
#include "packet/parameter/reconfiguration_response_parameter.h"
#include "packet/parameter/ssn_tsn_reset_request_parameter.h"
...
std::optional<Parameters> Parameters::Parse(
  ...
  while (!span.empty()) {
    if (span.size() < kParameterHeaderSize) {
      SCTP_DLOG_WARNING() << "Insufficient parameter length";
      return std::nullopt;
    }
    BoundedByteReader<kParameterHeaderSize> header(span);
    uint16_t length = header.Load16<2>();
    if (length < kParameterHeaderSize || length > span.size()) {
      SCTP_DLOG_WARNING() << "Invalid parameter length field";
      return std::nullopt;
    }
    size_t length_with_padding = RoundUpTo4(length);
    if (length_with_padding > span.size()) {
